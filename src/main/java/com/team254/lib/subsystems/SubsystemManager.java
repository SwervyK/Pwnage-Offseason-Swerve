...
public class SubsystemManager implements ILooper {
    ...
    public void executeEnabledLoopStarts(double timestamp){
        mAllSubsystems.forEach(l -> l.onEnabledLoopStart(timestamp));
    }

    public void executeEnabledLoops(double timestamp) {
        mAllSubsystems.forEach(Subsystem::readPeriodicInputs);
        mAllSubsystems.forEach(l -> l.onEnabledLoop(timestamp));
        mAllSubsystems.forEach(Subsystem::writePeriodicOutputs);
    }

    public void executeDisabledLoops(double timestamp) {
        mAllSubsystems.forEach(Subsystem::readPeriodicInputs);
        // mAllSubsystems.forEach(l -> l.onEnabledLoop(timestamp));
        // mAllSubsystems.forEach(Subsystem::writePeriodicOutputs);
    }

    public void executeEnabledLoopStops(double timestamp){
        mAllSubsystems.forEach(l -> l.onEnabledLoopStop(timestamp));
    }
