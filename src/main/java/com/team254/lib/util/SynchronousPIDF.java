...
public class SynchronousPIDF {
    ...
    private double m_maxTotalError = 0.0; // the sum of the errors for use in the integral calc
    ...
    public double calculate(double input, double dt) {
        ...
        if (Util.inRange(m_error * m_P, m_minimumOutput, m_maximumOutput)) {
            ...
            m_totalError = Util.limit(m_totalError, m_maxTotalError);
        } else {
            m_totalError = 0;
        }
        ...
        // System.out.println(proportionalError + " " + m_error + " " + m_deadband);
        m_result = (m_P * proportionalError) + (m_I * m_totalError) + (m_D * (m_error - m_prevError) / dt) + (m_F * Math.signum(m_error)); // changed how FF works 3/7/22 DRL
        // m_result = (m_P * proportionalError) + (m_I * m_totalError) + (m_D * (m_error - m_prevError) / dt) + (m_F * ((m_setpoint == 0) ? Math.signum(m_error) : m_setpoint)); // was m_F * m_setpoint (which was not good for when setpoint = 0)
    ...
    public void setIntegratorMax(double max_magnitude){
        m_maxTotalError = max_magnitude;
    }
