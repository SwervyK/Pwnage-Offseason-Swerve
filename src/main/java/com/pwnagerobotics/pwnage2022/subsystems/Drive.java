...
public class Drive extends Subsystem {
  ...
  public void setKinematicsDrive(final double throttle, final double strafe, final double rotationX, final double rotationY) {
    ...
    if (mCurrentRotationMode == RotationMode.FIELD) { // Point robot in direction of controller using pid
      ...
      double distance = SwerveDriveHelper.getDistance(mPeriodicIO.gyro_angle, wantedRobotAngle);
    ...
    if (rotationX == 0 && Math.abs(gyroDelta()) < Constants.kMinGyroDelta) {
      double distance = SwerveDriveHelper.getDistance(mPeriodicIO.gyro_angle, mWantedAngle);
    ...
    // if (/* Motors move and we dont */) {
      ...
      if (throttle == 0 && strafe == 0 && rotationX == 0 && rotationY == 0) {
        ...
        mModules[3].getDriveVelocity() < Constants.kDriveMinSpeed) {
          for (int i = 0; i < mModules.length; i++) {
            ...
            SwerveDriveHelper.getTurnAngle(i%2==0?Constants.kDriveWidth/2:-Constants.kDriveWidth/2, i<2?Constants.kDriveLength/2:-Constants.kDriveLength),0);
          }
        }
        return;
      }
      ...
      Object[][] module = Kinematics.inverseKinematics(controllerInputs, Math.toRadians(mPeriodicIO.gyro_angle), mCurrentDriveMode == DriveMode.FIELD);
    ...
    public void jukeMove(boolean jukeRight, boolean jukeLeft) {
      double[] robotCenter = SwerveDriveHelper.jukeMove(jukeRight, jukeLeft, mLastNonZeroRobotAngle - mPeriodicIO.gyro_angle, mRobotCenterX, mRobotCenterY);
      mRobotCenterX = robotCenter[0];
      mRobotCenterY = robotCenter[1];
    }
